...
class MagresParser(MatchingParser):
    ...
    def parse_model_system(self, logger: 'BoundLogger') -> Optional['MagresParser.model_system_class']:
        """
    ...
    def parse_magnetic_shieldings(
        self, magres_data: TextParser, cell: 'MagresParser.cell_class', logger: 'BoundLogger'
    ...
    def parse_nmr_magres_file_format(
        ...
        """
        workflow = self.workflow_class(method=self.workflow_method_class(), results=self.workflow_results_class())
        ...
        if input_structure:
            workflow.m_add_sub_section(
                self.workflow_class.inputs, Link(name='Input structure', section=input_structure)
            )
